/*** BeginCopyright
 * Copyright 2002, Georgia Institute of Technology, All Rights Reserved.
 * Unauthorized use and/or redistribution is disallowed.
 * This library is distributed without any warranty; without even
 * the implied warranty of fitness for a particular purpose.
 *
 * UAV Laboratory
 * School of Aerospace Engineering
 * Georgia Institute of Technology
 * Atlanta, GA 30332
 * http://controls.ae.gatech.edu
 *
 * Contact Information:
 * Prof. Eric N. Johnson
 * http://www.ae.gatech.edu/~ejohnson
 * Tel : 404 385 2519
 * EndCopyright
 ***/
/**
 *
 * Author: Joel Dunham (joel.dunham@gatech.edu), February 15, 2017
 *
 *
 * This set of functions handles AGL sensors
 *
 */

#ifndef _RMAX_AGLSENSOR_H_
#define _RMAX_AGLSENSOR_H_

#include "rmax/sensors_ref.h"

#if defined(__cplusplus)
extern "C"
{
#endif

/**
 * Updates all AGL sensors, deciding on outputs, etc
 * rangeFinderCmd is for the RangeFinder only; does not apply to the SF10/30/LTI systems
 * sonUpdateMode affects the output for certain update modes.
 */
void updateAglSensors(struct sensors_ref *sen, struct datalinkMessageRangeFinderCmd_ref* rangeFinderCmd,
					  double time, char sonUpdateMode);

static const unsigned char crc_table_terra[] = {};



#if defined(__cplusplus)
}
#endif	

#endif  //_RMAX_AGLSENSOR_H_